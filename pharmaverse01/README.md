...
| # | Lesson | Package(s) |
|---|---|---|
| 38 | `{teal}` Part 1: framework concepts | `teal` |
| 39 | `{teal}` Part 2: pre-built clinical modules | `teal.modules.clinical` |
| 40 | `{teal}` Part 3: building custom modules | `teal`, `teal.transform` |

### Module 9 — Submission and transport (`09_submission/`)

| # | Lesson | Package(s) |
|---|---|---|
| 41 | `{xportr}` — XPT v5 transport for FDA | `xportr` |
| 42 | `{datasetjson}` — CDISC Dataset-JSON, the XPT successor | `datasetjson` |

### Module 10 — Traceability, validation, and tooling (`10_traceability/`)

| # | Lesson | Package(s) |
|---|---|---|
| 43 | `{logrx}` — execution logging | `logrx` |
| 44 | `{diffdf}` — dataset comparison for dual programming | `diffdf` |
| 45 | `{riskmetric}` — assessing package risk for regulated use | `riskmetric` |

### Capstone (`99_capstone/`)

| # | Lesson | What you'll do |
|---|---|---|
| 46 | End-to-end walkthrough | Build a complete deliverable for a synthetic oncology study: raw EDC → SDTM → ADaM → ARDs → demographics + AE + KM table → teal app → XPT for submission |
...
| Module | Status |
|---|---|
| 0 — Intro | **Batch 1: in progress** |
| 1 — R foundations | **Batch 1: in progress** |
| 2 — Raw and SDTM | Pending |
| 3 — Metadata | Pending |
| 4 — ADaM core | Pending |
| 5 — ADaM TA extensions | Pending |
| 6 — TLG future (Cardinal) | Pending |
| 7 — TLG legacy | Pending |
| 8 — Shiny (teal) | Pending |
| 9 — Submission | Pending |
| 10 — Traceability | Pending |
| 99 — Capstone | Pending |
